...
class SistemaAcionistas {
    public List<String> obterAnalisesDesempenho(String dataInicialStr, String dataFinalStr) throws ParseException {
        ...
        return analises.stream()
                .filter(analise -> {
                    Date dataAnalise = analise.getData();// Obter a data da análise
                    return !dataAnalise.before(dataInicial) && !dataAnalise.after(dataFinal);
                })
                .map(analise -> analise.getDescricao().toString())
                .toList();
    }
